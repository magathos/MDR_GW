  ...
  for (dfile, lab) in zip(datafiles, labels):
    ...
    if "lambda_g" in data.dtype.names:
      ...
      if MASSPRIOR:
        ...
        weights = 1.0/lgdata**2
      else:
        weights = None
      figl_hist = plotPosteriorHist(lgdata, xlabel="$l_g$", label=lab, weights=weights)
      figl_kde, kde_lg = plotPosteriorKDE(lgdata, xlabel="$l_g$", label=lab)
      figlam_hist = plotPosteriorHist(lamgdata, xlabel="$\lambda_g$", label=lab, weights=weights)
      ...
      print "Plotting lg"
      figl_hist = plotPosteriorHist(lgdata, logplot=True, xmin=1e13, xmax=1e20, label=lab, weights=weights)
    ...
    #    figlam_hist = plotPosteriorHist(lamgdata, logplot=True, xmin=1e13, xmax=1e20, label=lab)
      ...
      logleffdata = array(data["log10lambda_eff"])
